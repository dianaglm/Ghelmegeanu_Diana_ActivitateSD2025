...
struct Heap {
	Masina* vector;
	int nrMasini; //pozitiile ocupate
	int lungime;
};
...
void filtreazaHeap(Heap heap, int pozitieNod) {
	...
	if (pozS < heap.nrMasini && heap.vector[pozD].id > heap.vector[pozMax].id) {
		pozMax = pozD;
	}
	if (pozMax != pozitieNod)
	{
		...
		if (pozMax < (heap.nrMasini - 2) / 2)
			filtreazaHeap(heap, pozMax);    // 2 * pozMax <= heap.nrMasini - 1]
	}
}
...
// o lista cu masinile din heap cu id mai mare ca o val primita ca parametru
...
void inserareLaInceput(Nod** lista, Masina m)
{
